...
$bot->command('notifications_off', function ($message) use ($bot) {
	...
	$keyboard = new \TelegramBot\Api\Types\ReplyKeyboardHide();
	$bot->sendMessage($message->getChat()->getId(), $answer, false, null, null, $keyboard);
	$query = "DELETE FROM bot_notification_public" . $db_name . " where chat_id={$message->getFrom()->getId()} and user_id={$message->getFrom()->getId()} and notification_type=1;\n";
	$result = pg_query($query) or $answer = 'Не удалось соединиться: ' . pg_last_error();
	$query = "DELETE FROM bot_notification_public" . $db_name . " where chat_id={$message->getFrom()->getId()} and user_id={$message->getFrom()->getId()} and notification_type=2;\n";
	$result = pg_query($query) or $answer = 'Не удалось соединиться: ' . pg_last_error();
	$query = "DELETE FROM bot_notification_public" . $db_name . " where chat_id={$message->getFrom()->getId()} and user_id={$message->getFrom()->getId()} and notification_type=3;\n";
	$result = pg_query($query) or $answer = 'Не удалось соединиться: ' . pg_last_error();
	$query = "DELETE FROM bot_notification_public" . $db_name . " where chat_id={$message->getFrom()->getId()} and user_id={$message->getFrom()->getId()} and notification_type=4;\n";
	$result = pg_query($query) or $answer = 'Не удалось соединиться: ' . pg_last_error();
});
...
/notifications_off - Отключение всех уведомленией (/notification для повторной настройки)
		...
		if (count($good_fights) > 0) {
			...
			for ($i = count($good_fights) - 1; $i >= 0; $i--) {
				...
				if ($clan_id == $good_fights[$i][winer_id]) {
					if ($clan_id == $good_fights[$i][attacker_id]) {
						$answer .= $t . ") " . $dt2 . ' МСК Мы отбили ' . $good_fights[$i][to] . ' у ' . $good_fights[$i][defender] . PHP_EOL;
					} else if ($clan_id == $good_fights[$i][defender_id]) {
						$answer .= $t . ") " . $dt2 . ' МСК Мы защитили ' . $good_fights[$i][to] . ' от ' . $good_fights[$i][attacker] . PHP_EOL;
					}
				} else if ($clan_id != $good_fights[$i][winer_id]) {
					if ($clan_id == $good_fights[$i][defender_id]) {
						$answer .= $t . ") " . $dt2 . ' МСК Мы отдали ' . $good_fights[$i][to] . ' клану ' . $good_fights[$i][attacker] . PHP_EOL;
					} else if ($clan_id == $good_fights[$i][attacker_id]) {
						$answer .= $t . ") " . $dt2 . ' МСК Мы не смогли отбить ' . $good_fights[$i][to] . ' у ' . $good_fights[$i][defender] . PHP_EOL;
					}
				}
				// if ($clan_id == $good_fights[$i][attacker_id]) {
